...
!KO          plan_ai = lun%canyon_hwr(l)/(lun%canyon_hwr(l) + 1._r8)
!KO 
...
!KO          build_lw_ratio = plan_ai

          ! Calculate frontal area index
!KO          frontal_ai = (1._r8 - plan_ai) * lun%canyon_hwr(l)

          ! Adjust frontal area index for different building configuration
!KO          frontal_ai = frontal_ai * sqrt(1/build_lw_ratio) * sqrt(plan_ai)
!KO
...
!KO
          write(iulog,*)'z_d_town: ',lun%z_d_town(l)
          write(iulog,*)'z_0_town: ',lun%z_0_town(l)
!KO
       else ! Not urban point 

!YS          this%eflx_traffic_factor(l) = spval
!YS
          this%nlane_traffic(l) = spval
